...
export default function VisualSelectionPuzzle({ 
  ...
  const handleSubmit = () => {
    ...
    if (!isCorrect) {
      // 選擇錯誤，顯示錯誤提示
      if (isMultiSelect) {
        setInternalError('這個組合不夠穩定。你需要選擇同時符合多個條件的固定點，形成穩定的三角支撐系統。');
      } else {
        const selectedOption = options.find(opt => opt.id === selected[0]);
        setInternalError(`「${selectedOption?.label || '這個固定點'}」不符合條件。請根據線索重新選擇。`);
      }
      return;
    }
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {options.map((option) => {
              ...
              const isCorrect = Array.isArray(puzzle.solution) 
                ? puzzle.solution.includes(option.id)
                : puzzle.solution === option.id;
